...
df = (
    ...
    .schema(dataSchema)
    .load("/mnt/volume/ggcluster/clusterdata-2011-2/task_usage/out.csv")

)
df.createOrReplaceTempView("dataFrame")
# df.printSchema()
sumCPUUsage = sql_context.sql("SELECT startTime, sum(mean_local_disk_space) from dataFrame group by startTime")
# sumCPUUsage.show(5000)
schema_df = ["startTime","numberOfJob"]
sumCPUUsage.toPandas().to_csv('./thangbk2209/results/startTime-localDiskSpace.csv', index=False, header=None)
