...
# df.printSchema()
sumCPUUsage = sql_context.sql("SELECT JobId, sum(mean_local_disk_space) from dataFrame group by JobId")
# sumCPUUsage.show(5000)
schema_df = ["startTime","numberOfJob"]
sumCPUUsage.toPandas().to_csv('./thangbk2209/results/test.csv', index=False, header=None)
